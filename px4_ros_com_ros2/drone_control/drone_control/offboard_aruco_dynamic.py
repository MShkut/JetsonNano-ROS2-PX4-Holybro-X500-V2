...
class OffboardAruco(Node):
    ...
    def aruco_poses_callback(self, msg):
        ...
        if self.aruco_x != 0.0 and self.aruco_y != 0.0 and self.aruco_detected == False:
    ...
    def timer_callback(self):
        ...
        if self.aruco_detected == True and 1.45 < self.curr_z < 1.55:
            self.get_logger().info('Acquiring aruco')
            self.current_setpoint_index = 2

        if self.aruco_detected == True and (self.curr_x - self.aruco_x) < 0.2 and (self.curr_y - self.aruco_y) < 0.2:
