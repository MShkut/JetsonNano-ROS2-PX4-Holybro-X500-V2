...
class OffboardAruco(Node):
    def __init__(self):
        ...
        self.aruco_small_detected = False
        ...
        self.aruco_large_x, self.aruco_large_y, self.aruco_small_x, self.aruco_small_y, self.z, self.yaw = 0.0, 0.0, 0.0, 0.0, -1.5, 0.0
    ...
    def aruco_poses_small_callback(self, msg):
        ...
        self.aruco_small_detected = True
    ...
    def timer_callback(self):
        ...
        if self.aruco_large_detected == True and self.current_setpoint_index == 1 or self.current_setpoint_index == 2:
            # Use PID controller to adjust setpoint based on error
            self.current_setpoint_index = 2
            self.get_logger().info('Acquiring large aruco')
            self.z += 0.005
            ...
            if self.z > 0.05 and -self.curr_z > 0.05:
                self.land()
        
        if self.aruco_large_detected == True and self.aruco_small_detected == True and self.curr_z < 0.6:
            # Use PID controller to adjust setpoint based on error
            self.get_logger().info('Acquiring small aruco')
            self.z += 0.005
            self.control_x = self.PID_calculate(self.curr_x - self.aruco_small_x)
            self.control_y = self.PID_calculate(self.curr_y - self.aruco_small_y)
            self.setpoints[2] = (self.aruco_small_x + self.control_x, -self.aruco_small_y + self.control_y, self.z, self.yaw)
            if self.z > 0.05 and -self.curr_z > 0.05:
                self.land()
