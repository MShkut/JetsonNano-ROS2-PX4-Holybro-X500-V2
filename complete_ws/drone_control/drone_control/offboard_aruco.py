...
class OffboardAruco(Node):
    def __init__(self):
        ...
        self.aruco_x, self.aruco_y, self.aruco_z, self.aruco_yaw = 0.0, 0.0, 0.0, 0.0
        self.setpoints = [
            (self.zero_x, self.zero_y, -2.0, self.zero_yaw),
            (self.zero_x, self.zero_y, self.zero_z, self.zero_yaw),
            (self.aruco_x, self.aruco_y, self.aruco_z, self.aruco_yaw)
        ]
        ...
        self.aruco_pose_large_subscriber = self.create_subscription(
            PoseArray, 'aruco_poses_large', self.aruco_poses_large_callback, 10
        )
        '''self.aruco_pose_small_subscriber = self.create_subscription(
    ...
    def update_setpoint(self):
        if self.aruco_x is not None:
            ...
            self.get_logger().info('Setpoint 2')
        else:
    ...
    def aruco_poses_small_callback(self, msg):
        for pose in msg.poses:
            if self.aruco_z <= -0.5: 
                ...
                orientation = euler_from_quaternion(orientation_q)
                self.aruco_yaw = orientation[2]
            if  (self.aruco_x - pose.position.x) < 0.01 and (self.aruco_y - pose.position.y) < 0.01:
                self.aruco_x = pose.position.x
                self.aruco_y = pose.position.y
                self.aruco_z = 0.29
                orientation_q = [pose.orientation.x, pose.orientation.y, pose.orientation.z, pose.orientation.w]
